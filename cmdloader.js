...
const ownerFiles = await readdir("./commands/Owner/");
...
ownerFiles.forEach(f => {
    if(!f.endsWith(".js"))return;
		let owner = require(`../commands/Owner/${f}`);
    client.ownerCommands.set(owner.help.name, owner);
    client.commands.set(owner.help.name, owner);
	client.aliases.set(owner.help.alias, owner);
	});
